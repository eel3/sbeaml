/* ********************************************************************** */
/**
 * @brief   SBEAML: sample && test application.
 * @author  eel3
 * @date    2017-09-03
 */
/* ********************************************************************** */
...
using EVENT_ID_MAKER = SBEAML_EVENT_ID (*)(const VS& argv);
...
/** Module context type. */
struct MODULE_CTX {
    Mailbox<SBEAML_EVENT_ID> mailbox;
};
...
/* ====================================================================== */
SBEAML_EVENT_ID
command_push_handler(const VS& argv)
{
    if (argv.size() != 1) {
        return SBEAML_EVENT_ID_NONE;
    }

    auto bits = EVENT_BIT_CMD_PUSH_HANDLER;

    return static_cast<SBEAML_EVENT_ID>(bits);
}

/* ====================================================================== */
/**
 * @brief  Returm event ID for "pop-handler" command.
 *
 * @param[in] argv  Command name and arguments.
 *
 * @return Event ID.
...
/* ====================================================================== */
SBEAML_EVENT_ID
command_pop_handler(const VS& argv)
{
    if (argv.size() > 2) {
        return SBEAML_EVENT_ID_NONE;
    }
    ...
    if (argv.size() == 1) {
        type = EVENT_BIT_POP_TYPE_ONE;
    } else if (argv[1] == "all") {
        type = EVENT_BIT_POP_TYPE_ALL;
    } else {
        const auto op_type = argv[1];
        try {
            auto n = std::stoul(op_type);
            if (n > static_cast<unsigned long>(EVENT_BITMASK_POP_TAG)) {
                throw std::range_error("");     // XXX Dirty hack.
            }
            type = EVENT_BIT_POP_TYPE_TAG;
            tag = static_cast<EVENT_BIT>(n) & EVENT_BITMASK_POP_TAG;
        } catch (...) {
            std::cerr << cmd << ": invalid option: " << op_type << std::endl;
            return SBEAML_EVENT_ID_NONE;
        }
    }

    auto bits = EVENT_BIT_CMD_POP_HANDLER | type | tag;
...
command_set_timer(const VS& argv)
{
    ...
    EVENT_BIT id;
    if (!convert_to_timer_id(opt_id, SBEAML_CFG_MAX_TIMER, id)) {
...
command_set_gtimer(const VS& argv)
{
    ...
    EVENT_BIT id;
    if (!convert_to_timer_id(opt_id, SBEAML_CFG_MAX_GLOBAL_TIMER, id)) {
        std::cerr << cmd << ": invalid id: " << opt_id << std::endl;
        return SBEAML_EVENT_ID_NONE;
    }
...
#define ENTRY(cmd, fn, arg, desc) \
    ...
    ENTRY(push-handler,   command_push_handler, "(no option)",                       "Push next event handler."),
    ENTRY(pop-handler,    command_pop_handler,  "[(no option)|tag-number|all]",      "Pop handlers."),
...
main_loop(std::promise<bool>& pr_init, std::future<void>& fu_fin)
{
    ...
    err = sbeaml_Initialize();
    if (err != SBEAML_E_OK) {
        pr_init.set_value(false);
        return;
    }

    SBEAML_PREPARE_PARAMS params { &root_event_handler };

    err = sbeaml_PrepareBeforeMainLoop(&params);
    if (err != SBEAML_E_OK) {
        sbeaml_Finalize();
        pr_init.set_value(false);
        return;
...
post_message()
{
    ...
    SBEAML_MESSAGE msg = default_message;
    msg.user_data = (void *) prefix;
    return sbeaml_PostMessage(&msg) == SBEAML_E_OK;
}

} // namespace

/* ---------------------------------------------------------------------- */
/* SBEAML: machdep implementation */
/* ---------------------------------------------------------------------- */
...
main()
{
    ...
    if (!initialized) {
        th.join();
        cerr << "Failed to initialize sbeaml module." << endl;
        return EXIT_FAILURE;
    }
    ...
    while (cr.read()) {
        ...
        if (event_id == SBEAML_EVENT_ID_NONE) {
            show_command_help(*p);
            continue;
        }
        auto& mc = module_ctx;
